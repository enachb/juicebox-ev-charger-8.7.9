...
int uLCD_144_SPE::waitAck() {
  ...
  do {
    ...
    delay(5);
    z++;
  } while(x!=0x06 && z<200); // wait for screen acknowledgement or 200 cycles
...
void uLCD_144_SPE::setMode(int mode) {
  Serial.print((char)0xFF);
...
void uLCD_144_SPE::printStr(int col, int row, int font, byte red, byte green, byte blue, const __FlashStringHelper *str) {
  moveCursor(col, row);
  setFont(0);
  setFGcolor(red, green, blue);
  setYspacing(2);
